    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.conv33conv33conv11 = nn.Sequential(
            nn.Conv2d(hidden_dim // 2, hidden_dim // 2, kernel_size=3, stride=1, padding=1, dilation=1, groups=hidden_dim // 2),
            nn.Conv2d(hidden_dim // 2, hidden_dim // 2, kernel_size=1),
            nn.BatchNorm2d(hidden_dim // 2),
            nn.ReLU(),
            nn.Conv2d(hidden_dim // 2, hidden_dim // 2, kernel_size=3, stride=1, padding=2, dilation=2, groups=hidden_dim // 2),
            nn.Conv2d(hidden_dim // 2, hidden_dim // 2, kernel_size=1),
            nn.BatchNorm2d(hidden_dim // 2),
            nn.ReLU(),
            nn.Conv2d(hidden_dim // 2, hidden_dim // 2, kernel_size=3, stride=1, padding=3, dilation=3, groups=hidden_dim // 2),
            nn.Conv2d(hidden_dim // 2, hidden_dim // 2, kernel_size=1),
            nn.BatchNorm2d(hidden_dim // 2),
            nn.ReLU(),
            nn.Conv2d(hidden_dim // 2, hidden_dim // 2, kernel_size=3, stride=1, padding=1, dilation=1, groups=hidden_dim // 2),
...
class HCLayer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if True: # is this really applied? Yes, but been overriden later in HCM!
...
class HCM(nn.Module):
    ...
    def _init_weights(self, m: nn.Module):
        """
        out_proj.weight which is previously initilized in HCBlock, would be cleared in nn.Linear
        no fc.weight found in the any of the model parameters
        no nn.Embedding found in the any of the model parameters
        so the thing is, HCBlock initialization is useless
